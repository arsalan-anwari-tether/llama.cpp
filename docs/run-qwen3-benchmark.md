...
The script uses `--quiet` mode to suppress all llama-cli console UI output (banner, loading message, prompt echo, timing info, etc.) so only the model's actual response is written to stdout and captured in the markdown files.
